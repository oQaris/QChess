...
public class MoveSystem {
    ...
    public Figure move(Move move) throws ChessError {
        try {
            // взятие на проходе
            if (move.getMoveType() == MoveType.EN_PASSANT) board.removeFigure(history.getPrevMove().getTo());
    ...
    private boolean isCorrectVirtualMove(Move move) throws ChessError, ChessException {
        ...
        if (virtualKilled != null && virtualKilled.getType() == TypeFigure.KING) throw new ChessError(KING_NOT_FOUND);
