...
public class Pawn extends Figure {
  ...
  private void addShortAndLongMove(GameSettings settings, Cell forwardShift, Set<Move> result) {
    ...
    if (settings.board.isEmptyCell(move)) {
      result.add(
          new Move(isTurnInto(move) ? MoveType.TURN_INTO : MoveType.QUIET_MOVE, position, move));
  ...
  /**
   * Проверяет, является ли атака пешки взятием на проходе. Входные данные должны гарантировать, что
   * это именно атака пешки (диагональный ход)
   *
