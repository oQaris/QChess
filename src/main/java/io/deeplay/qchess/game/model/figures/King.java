...
public class King extends Figure {
  ...
  public Set<Move> getAttackedMoves(Board board) {
    return stepForEach(
        board, Stream.concat(xMove.stream(), plusMove.stream()).collect(Collectors.toList()));
  }
  ...
  private boolean isCorrectCastling(GameSettings settings, boolean shortCastling) {
    if (wasMoved
        ...
        || Board.isAttackedCell(
            settings, position.createAdd(new Cell(shortCastling ? 1 : -1, 0)), color.inverse())
        || Board.isAttackedCell(
            settings, position.createAdd(new Cell(shortCastling ? 2 : -2, 0)), color.inverse())) {
      return false;
    }
    try {
      Figure rook =
          settings.board.getFigure(position.createAdd(new Cell(shortCastling ? 3 : -4, 0)));
