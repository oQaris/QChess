...
public class InputTrafficHandler extends Thread {
    ...
    public InputTrafficHandler(
            final Socket socket,
            final Consumer<ServerToClientDTO> setLastResponse,
            final Supplier<Boolean> waitForResponse)
    ...
    private InputStream tryGetSocketInput(final Socket socket) throws ClientException {
        try {
    ...
    private OutputStream tryGetSocketOutput(final Socket socket) throws ClientException {
        try {
    ...
    private void inputTrafficHandlerUpdate() throws IOException {
        if (in.ready()) {
            ...
            final String response = TrafficRequestHandler.process(request);
            if (waitForResponse.get()) {
                try {
                    setLastResponse.accept(SerializationService.serverToClientDTOMain(request));
                } catch (final SerializationException ignore) {
